...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle)
{
	...
	}else
	{
		...
		SYSCFG_PCLK_EN();
		SYSCFG->EXTICR[temp1] &= ~(0xF << (temp2 * 4));   // clear the 4-bit field
		SYSCFG->EXTICR[temp1] |=  (portcode << (temp2 * 4)); // write the correct port code
 ...
 * @Note			-none
 */
void GPIO_Delnit(GPIO_RegDef_t *pGPIOx)
{
	if(pGPIOx == GPIOA)
			{
				GPIOA_REG_RESET();
